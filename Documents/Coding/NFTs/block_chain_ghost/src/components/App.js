...
// Import ABI + Config
import BlockChainGhost from '../abis/BlockChainGhost.json'
import config from '../config.json'

function App() {
	const [web3, setWeb3] = useState(null)
	const [BlockChainGhost, setBlockChainGhost] = useState(null)
	...
	const loadBlockchainData = async (_web3, _account, _networkId) => {
		...
		try {
			const BlockChainGhost = new _web3.eth.Contract(BlockChainGhost.abi, BlockChainGhost.networks[_networkId].address)
			setBlockChainGhost(BlockChainGhost)

			const maxSupply = await BlockChainGhost.methods.maxSupply().call()
			const totalSupply = await BlockChainGhost.methods.totalSupply().call()
			setSupplyAvailable(maxSupply - totalSupply)

			const allowMintingAfter = await BlockChainGhost.methods.allowMintingAfter().call()
			const timeDeployed = await BlockChainGhost.methods.timeDeployed().call()
			setRevealTime((Number(timeDeployed) + Number(allowMintingAfter)).toString() + '000')

			if (_account) {
				const ownerOf = await BlockChainGhost.methods.walletOfOwner(_account).call()
				setOwnerOf(ownerOf)
				console.log(ownerOf)
			} else {
				setOwnerOf([])
	...
	const mintNFTHandler = async () => {
		...
		// Mint NFT
		if (BlockChainGhost && account) {
			setIsMinting(true)
			setIsError(false)

			await BlockChainGhost.methods.mint(1).send({ from: account, value: 0 })
				.on('confirmation', async () => {
					const maxSupply = await BlockChainGhost.methods.maxSupply().call()
					const totalSupply = await BlockChainGhost.methods.totalSupply().call()
					setSupplyAvailable(maxSupply - totalSupply)

					const ownerOf = await BlockChainGhost.methods.walletOfOwner(account).call()
					setOwnerOf(ownerOf)
				})
				.on('error', (error) => {
					window.alert(error)
				...
				<section id='welcome' className='welcome'>
					...
					<Row className='header my-3 p-3 mb-0 pb-0'>
						<Col xs={12} md={12} lg={8} xxl={8}>
							<h1>BLOCK CHAIN GHOST</h1>
							<p className='sub-header'>Availble on 04 / 25 / 22</p>
						</Col>
						<Col className='flex social-icons'>
							<a
								href="https://twitter.com/blockchainghost"
								target='_blank'
					...
					<Row className='flex m-3'>
						...
						<Col md={5} lg={4} xl={5} xxl={4}>
							...
							<p className='text'>
								Block Chain Ghost NFT Collection
							</p>
				...
				<section id='about' className='about'>
						...
						<Col md={5} lg={4} xl={5} xxl={4}>
							{isError ? (
								...
								<div>
									...
									<ul>
										<li>2,500 generated Block Chain Ghost art generator</li>
									...
									{ownerOf.length > 0 &&
										<p><small>View your NFT on
											<a
												href={`${openseaURL}/assets/${BlockChainGhost._address}/${ownerOf[0]}`}
												target='_blank'
					...
					<Row style={{ marginTop: "100px" }}>
						<Col>
							{BlockChainGhost &&
								<a
									href={`${explorerURL}/address/${BlockChainGhost._address}`}
									target='_blank'
									className='text-center'>
									{BlockChainGhost._address}
								</a>
							}
						</Col>
					</Row>
